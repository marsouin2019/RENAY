...
class Personne
{
    ...
    public function add_personne(array $data)
    {
       ...
       if ($resultat == 1) 
       {
            ...
            {
                // insert login
                $resultat = $this->connectDB->query( "INSERT INTO logins (idpersonne, password, email )VALUES('".$idP ."', '".$this->password."', '".$this->email."')");
    ...
    public function login(array $data){
        ...
        if (strlen($data["frmPass"]) < 8 ) 
        {
            ...
            echo false;
                ...
                if (password_verify($data["frmPass"],$password)) 
                {
                    ...
                    header('Location: /eval-e2.com/admin/index.php'); 
                    

                }else{
                    //redirection vers page form connexion
                     }
                }
            }
      ...
      public function all_personnes()
      {
          ...
          */
          ...
          for($i=0;  $i < $nbr; $i++)
          {
              //stocker le resultat boucle dans variabke de type array (à)
              $rowA[] = $requete->fetch_assoc();
          }
          return $rowA;
      }
